...
import ConfigDialog from "../../chatbot/config-dialog";
import ImageDialog from "../../chatbot/image-dialog";
import {
  ChabotInputModality,
...
export interface EcisoChatInputPanelProps {
  running: boolean;
...
export abstract class EcisoChatScrollState {
...
export default function EcisoChatInputPanel(props: EcisoChatInputPanelProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.log("~~~~Eciso Override~~~~")
        const eciso_override = [
          {
  ...
  useEffect(() => {
    const onWindowScroll = () => {
      if (EcisoChatScrollState.skipNextScrollEvent) {
        EcisoChatScrollState.skipNextScrollEvent = false;
        return;
      }
      ...
      if (!isScrollToTheEnd) {
        EcisoChatScrollState.userHasScrolled = true;
      } else {
  ...
  useLayoutEffect(() => {
    if (EcisoChatScrollState.skipNextHistoryUpdate) {
      EcisoChatScrollState.skipNextHistoryUpdate = false;
      return;
    }

    if (!EcisoChatScrollState.userHasScrolled && props.messageHistory.length > 0) {
